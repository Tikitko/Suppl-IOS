...
class TracklistManager {
    ...
    private static func updateIfNeeded(_ needUpdate: Bool = false, ikey: Int, akey: Int, callback: @escaping (Bool) -> ()) {
        if inUpdate { return }
        inUpdate = true
        if !needUpdate {
        ...
        APIManager.tracklistGet(ikey: ikey, akey: akey) { error, data in
            ...
            if let tracklist = TracklistManager.tracklist ,tracklist == data.list {
                sendCallbackStatus(false, callback: callback)
                return
            }
            TracklistManager.tracklist = data.list
            sendCallbackStatus(true, callback: callback)
        }
    }
    ...
    public static func add(trackId: String, to: Int = 0, callback: @escaping (Bool) -> ()) {
        guard let ikey = UserDefaultsManager.identifierKey,
            let akey = UserDefaultsManager.accessKey else {
            sendCallbackStatus(false, callback: callback)
            return
        }
        updateIfNeeded(ikey: ikey, akey: akey) { updated in
            guard let tracklist = TracklistManager.tracklist else {
                sendCallbackStatus(false, callback: callback)
                return
            }
    ...
    public static func remove(from: Int = 0, callback: @escaping (Bool) -> ()) {
        ...
        updateIfNeeded(ikey: ikey, akey: akey) { updated in
            guard let tracklist = TracklistManager.tracklist, from <= tracklist.count - 1 else {
                sendCallbackStatus(false, callback: callback)
                return
            }
    ...
    public static func move(from: Int = 0, to: Int = 0, callback: @escaping (Bool) -> ()) {
        ...
        updateIfNeeded(ikey: ikey, akey: akey) { updated in
            guard let tracklist = TracklistManager.tracklist, from <= tracklist.count - 1, to <= tracklist.count else {
                sendCallbackStatus(false, callback: callback)
                return
            }
